...
TARGET_W = 200
TARGET_H = 200
RAW_BYTE_COUNT = (TARGET_W * TARGET_H) // 8  # 5,000 bytes
...
def convert_image_to_1bit_data(
    ...
    Restituisce:
      ...
      - pil_1bit: oggetto PIL Image a 1-bit per anteprima / salvataggio BMP.
    """
    img = Image.open(img_path).convert("RGBA")

    # Applica sfondo bianco su trasparenza
    bg = Image.new("RGBA", img.size, (255, 255, 255, 255))
    img = Image.alpha_composite(bg, img).convert("L")

    # Ridimensionamento
    if mode == "cover":
        # Scala e ritaglia al centro
        img = ImageOps.fit(img, (TARGET_W, TARGET_H), method=Image.Resampling.LANCZOS)
    elif mode == "contain":
        # Scala preservando proporzioni con barre bianche
    ...
    for y in range(TARGET_H):
        for x in range(TARGET_W):
            pixel_val = pixels[x, y]  # 0 = nero, 255 = bianco in PIL "1"
            is_black = (pixel_val == 0)
...
def find_sd_screensaver_dir() -> Optional[Path]:
    """Cerca automaticamente la cartella /screensavers/ su schede MicroSD montate."""
    candidates = [
...
def generate_sample_screensavers(out_dir: Path):
    ...
    for idx, (title, text, footer) in enumerate(quotes, 1):
        img = Image.new("1", (TARGET_W, TARGET_H), 1)  # Bianco
        ...
        # Converti in raw bytes
        pixels = img.load()
        raw = bytearray(RAW_BYTE_COUNT)
        for y in range(TARGET_H):
            for x in range(TARGET_W):
                if pixels[x, y] == 0:  # Nero
                    byte_idx = (y * TARGET_W + x) // 8
                    bit_idx = 7 - (x % 8)
...
def main():
    parser = argparse.ArgumentParser(
        description="ES1 Screensaver Converter — Converte immagini in formato 200x200 1-bit per E-Paper."
    )
    ...
    parser.add_argument("--mode", choices=["cover", "contain", "stretch"], default="cover", help="Modalità ridimensionamento (default: cover).")
    parser.add_argument("--no-dither", action="store_true", help="Disabilita il dithering Floyd-Steinberg e usa soglia fissa.")
    parser.add_argument("--threshold", type=int, default=128, help="Soglia bianco/nero quando dithering e' disabilitato (0-255).")
    ...
    if not args.input:
        parser.print_help()
        print("\n💡 Esempio: python img_to_screensaver.py wallpaper.jpg")
        print("💡 Oppure genera campioni: python img_to_screensaver.py --samples")
        return

    in_path = Path(args.input)
    if not in_path.exists():
        print(f"❌ Errore: Percorso non trovato: {in_path}")
        sys.exit(1)
    ...
    print(f"\n🚀 Conversione di {len(files_to_convert)} immagine/i per display ES1 (200x200 1-bit)...")

    sd_target_dir = Path(args.sd) if args.sd else find_sd_screensaver_dir()
    if sd_target_dir:
        print(f"💾 Destinazione MicroSD rilevata: {sd_target_dir}")
    ...
    for img_file in files_to_convert:
        try:
            ...
            print(f"  ✓ {img_file.name} ➔ {bmp_path.name} & {bin_path.name} (5.0 KB)")

            # Copia su SD se disponibile
            if sd_target_dir and sd_target_dir.exists():
                (sd_target_dir / bin_path.name).write_bytes(raw_bytes)
                print(f"    ↳ Copiato su SD: {sd_target_dir / bin_path.name}")

        except Exception as e:
            print(f"  ❌ Errore durante la conversione di {img_file.name}: {e}")

    print(f"\n✨ Conversione completata! I file sono pronti in: {out_dir}\n")
